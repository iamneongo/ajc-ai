...
function ImageManagerContent() {
  ...
  const loadImages = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Tải hình ảnh thất bại");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSetTags = async (item: ManagedImage, tags: string[]) => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Gắn thẻ thất bại");
    }
  };
  ...
  const handleDeleteTag = async (tag: string) => {
    try {
      ...
      toast.success(`Đã xóa thẻ "${tag}", ảnh hưởng ${result.removed_from} hình ảnh`);
    } catch (error) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success(`Đã xóa ${data.removed} hình ảnh`);
      setDeleteMode(null);
      setSelectedPaths([]);
      await loadImages();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Xóa hình ảnh thất bại");
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <section className="space-y-5">
      ...
      <Dialog open={dialogVisible} onOpenChange={(open) => { if (!open) closeDialog(); }}>
        <DialogContent className="max-w-sm overflow-hidden rounded-2xl">
          ...
          <p className="text-sm text-stone-600">
            Bạn có chắc muốn xóa hình ảnh này không? Hành động này không thể khôi phục.
          </p>
          {deleteTarget ? (
      ...
      <Dialog open={Boolean(deleteMode)} onOpenChange={(open) => (!open ? setDeleteMode(null) : null)}>
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            <DialogTitle>{deleteMode === "filtered" ? "Xóa ảnh theo ngày khớp" : "Xóa ảnh đã chọn"}</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-stone-600">
            Bạn có chắc muốn xóa {selectedCount} hình ảnh không? Hành động này không thể khôi phục.
          </p>
          <DialogFooter>
      ...
      <Dialog open={Boolean(tagDeleteTarget)} onOpenChange={(open) => { if (!open) setTagDeleteTarget(null); }}>
        <DialogContent className="max-w-sm rounded-2xl">
          ...
          <p className="text-sm text-stone-600">
            Bạn có chắc muốn xóa thẻ <span className="font-semibold">"{tagDeleteTarget}"</span> không? Thẻ này sẽ bị gỡ khỏi tất cả hình ảnh.
          </p>
          <DialogFooter>
