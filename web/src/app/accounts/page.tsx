...
const accountStatusOptions: { label: string; value: AccountStatus | "all" }[] = [
  ...
  { label: "Hoạt động", value: "正常" },
  { label: "Bị giới hạn", value: "限流" },
  { label: "Lỗi", value: "异常" },
  { label: "Đã vô hiệu hóa", value: "禁用" },
];
...
const metricCards = [
  ...
  { key: "active", label: "Tài khoản hoạt động", color: "text-emerald-600", icon: CheckCircle2 },
  { key: "limited", label: "Tài khoản bị giới hạn", color: "text-orange-500", icon: CircleAlert },
  { key: "abnormal", label: "Tài khoản lỗi", color: "text-rose-500", icon: CircleOff },
...
function formatRestoreAt(value?: string | null) {
  ...
  const relative = diffMs > 0 ? `Còn ${days}d ${hours}h` : "Đã đến thời gian khôi phục";
...
function AccountsPageContent() {
  ...
  const loadAccounts = async (silent = false) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Tải tài khoản thất bại";
      toast.error(message);
    } finally {
      if (!silent) {
        setIsLoading(false);
      ...
      <section className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            Làm mới toàn bộ tài khoản và hạn mức
          </Button>
          <AccountImportDialog
            disabled={isLoading || isRefreshing || isDeleting}
            onImported={(items) => {
        ...
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              Chỉnh sửa trạng thái tài khoản thủ công.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="flex flex-col gap-3 border-b border-stone-100 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-wrap items-center gap-2 text-sm text-stone-500">
                ...
                >
                  ...
                  Làm mới thông tin và hạn mức của tài khoản đã chọn
                </Button>
                <Button
                  variant="ghost"
              ...
              <table className="w-full min-w-[920px] text-left">
                <thead className="border-b border-stone-100 text-[11px] text-stone-400 uppercase tracking-[0.18em]">
                  <tr>
                    ...
                    <th className="w-24 px-4 py-3">Tác vụ</th>
                  </tr>
                </thead>
                <tbody>
              ...
              {!isLoading && currentRows.length === 0 ? (
                <div className="flex flex-col items-center justify-center gap-3 px-6 py-14 text-center">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-stone-500">Hãy điều chỉnh bộ lọc hoặc từ khóa tìm kiếm rồi thử lại.</p>
                  </div>
                </div>
              ) : null}
            </div>
