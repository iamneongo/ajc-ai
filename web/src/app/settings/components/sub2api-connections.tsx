...
export function Sub2APIConnections() {
    ...
    pollTimerRef.current = window.setInterval(() => {
      void fetchSub2APIServers()
        ...
        .catch((error) => {
          ...
          toast.error(error instanceof Error ? error.message : "Kiểm tra tiến độ nhập thất bại");
        });
    }, 1500);
  ...
  const handleFetchGroups = async () => {
    ...
    try {
      ...
      if (data.groups.length === 0) {
        toast.message("Phía remote chưa cấu hình nhóm");
      } else {
        toast.success(`Đã đọc ${data.groups.length} nhóm`);
      }
    } catch (error) {
  ...
  const handleBrowseAccounts = async (server: Sub2APIServer) => {
    ...
    try {
      ...
      setAccountPage(1);
      setBrowserOpen(true);
      toast.success(`Đọc thành công, có ${accounts.length} tài khoản OpenAI`);
    } catch (error) {
  ...
  const handleStartImport = async () => {
    ...
    try {
      ...
      toast.success("Đã bắt đầu tác vụ nhập");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Bắt đầu nhập thất bại");
    } finally {
      ...
      <Card className="rounded-2xl border-white/80 bg-white/90 shadow-sm">
        <CardContent className="space-y-6 p-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-stone-500">
                  Sau khi cấu hình máy chủ Sub2API, bạn có thể truy vấn tài khoản OpenAI OAuth và nhập hàng loạt vào hồ tài khoản cục bộ.
                </p>
              </div>
            </div>
                          ...
                          return (
                            <div className="rounded-lg border border-stone-200 bg-white px-3 py-3">
                              <div className="flex items-center justify-between gap-3">
                                <div className="min-w-0">
                                  <div className="text-sm font-medium text-stone-700">
                                    Trang thai {importJob.status}, da xu ly {importJob.completed}/{importJob.total}
                                  </div>
                                  <div className="truncate text-xs text-stone-400">
                                    Tac vu {importJob.job_id.slice(0, 8)} · {importJob.created_at}
                                  </div>
                                </div>
                                <Badge
                                  variant={
                              ...
                              <div className="mt-2 flex flex-wrap gap-2 text-xs text-stone-500">
                                ...
                                <span>Bo qua {importJob.skipped}</span>
                                <span>Lam moi {importJob.refreshed}</span>
          ...
          <div className="rounded-xl bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-500">
            <p className="font-medium text-stone-600">Huong dan</p>
            <ul className="mt-1 list-inside list-disc space-y-0.5">
              <li>Nhập địa chỉ Sub2API và tài khoản quản trị (hoặc Admin API Key), sau đó lưu thành một kết nối.</li>
              <li>Bấm "Đồng bộ" ở một kết nối để tải danh sách tài khoản có `platform=openai` và `type=oauth`.</li>
              <li>Sau khi tick các tài khoản cần thiết, backend sẽ đồng thời lấy `access_token`, tự động nhập vào hồ tài khoản cục bộ và làm mới trạng thái.</li>
              <li>Hệ thống chỉ đọc `access_token` từ thông tin đăng nhập sub2api; các trường như `refresh_token` sẽ không được ghi vào máy cục bộ.</li>
            </ul>
          </div>
        </CardContent>
      </Card>
        ...
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              {editingServer ? "Chỉnh sửa thông tin kết nối Sub2API" : "Thêm một kết nối Sub2API mới"}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Ví dụ: sub2api tự host"
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-stone-700">Kiểu xác thực</label>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      placeholder={editingServer ? "Để trống để giữ nguyên mật khẩu" : "Mật khẩu quản trị"}
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder={editingServer ? "Để trống để giữ nguyên khóa" : "Sub2API Admin API Key"}
                    className="h-11 rounded-xl border-stone-200 bg-white pr-10"
                  />
                  <button
                    type="button"
            ...
            <div className="space-y-2">
              ...
              {editingServer ? (
                <div className="flex items-center justify-between gap-2 text-xs text-stone-500">
                  <span>Khi đồng bộ, hệ thống sẽ lọc theo ID nhóm; để trống = đồng bộ tất cả tài khoản OpenAI OAuth.</span>
                  <Button
                    variant="outline"
                  ...
                  >
                    ...
                    {remoteGroups ? "Tải lại nhóm" : "Tải danh sách nhóm"}
                  </Button>
                </div>
              ) : (
                <div className="text-xs text-stone-500">
                  Sau khi thêm kết nối, bạn có thể bấm "Tải danh sách nhóm" trong hộp thoại sửa để chọn nhóm cụ thể.
                </div>
              )}
            </div>
          </div>
          <DialogFooter className="pt-2">
            ...
            >
              ...
              {editingServer ? "Luu thay doi" : "Them"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={browserOpen} onOpenChange={setBrowserOpen}>
        <DialogContent showCloseButton={false} className="max-h-[90vh] max-w-5xl rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              {browserServer ? `Tu ${browserServer.name || browserServer.base_url}` : "Tai khoan OpenAI OAuth tren Sub2API"}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="relative min-w-[260px]">
              ...
              <Input
                ...
                placeholder="Tim email, goi cuoc hoac ten"
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {allFilteredSelected ? "Bo chon tat ca" : "Chon tat ca ket qua loc"}
              </Button>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-stone-200">
            <div className="flex items-center justify-between border-b border-stone-100 px-4 py-3 text-sm text-stone-500">
              <div className="flex items-center gap-3">
                ...
                />
                <span>Ket qua loc {filteredAccounts.length} muc</span>
              </div>
              <span>Da chon {selectedIds.length} muc</span>
            </div>
            <div className="max-h-[420px] overflow-auto">
              {pagedAccounts.length === 0 ? (
                <div className="flex items-center justify-center py-12 text-sm text-stone-400">Không có tài khoản nào khớp</div>
              ) : (
                  ...
                  {pagedAccounts.map((item) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="truncate text-xs text-stone-400">
                          id {item.id}
                          {item.expires_at ? ` · Het han ${item.expires_at}` : ""}
                        </div>
                      </div>
                    </label>
                  ))}
          ...
          <div className="flex items-center justify-between text-sm text-stone-500">
            <span>
              Hien thi {filteredAccounts.length === 0 ? 0 : (safeAccountPage - 1) * currentPageSize + 1} -{" "}
              {Math.min(safeAccountPage * currentPageSize, filteredAccounts.length)} / {filteredAccounts.length}
            </span>
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
