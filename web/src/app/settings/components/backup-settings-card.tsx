...
const includeLabels: Array<{ key: keyof BackupInclude; label: string }> = [
  ...
  { key: "logs", label: "Nhật ký điều độ và gọi API" },
  { key: "image_tasks", label: "Bản ghi tác vụ hình ảnh" },
  { key: "accounts_snapshot", label: "Snapshot tài khoản" },
  { key: "auth_keys_snapshot", label: "Snapshot khóa người dùng" },
...
export function BackupSettingsCard() {
  ...
  const handleOpenDetail = async (key: string) => {
    ...
    try {
      ...
      setDetail(null);
      toast.error(error instanceof Error ? error.message : "Đọc chi tiết backup thất bại");
    } finally {
  ...
  const handleDownload = async (key: string, name: string) => {
    try {
      ...
      if (!authKey) {
        toast.error("Phiên đăng nhập hiện tại đã hết hạn, vui lòng đăng nhập lại trước khi tải");
        return;
      }
      ...
      if (!response.ok) {
        let message = "Tải backup thất bại";
        try {
          ...
          message = response.status === 401 ? "Đăng nhập đã hết hạn, vui lòng đăng nhập lại rồi thử lại" : message;
        }
      ...
      toast.success("Đã bắt đầu tải backup");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Tải backup thất bại");
    }
  };
        ...
        <CardContent className="space-y-6 p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold tracking-tight">Quản lý backup R2</h2>
                <p className="text-sm text-stone-500">Backup dữ liệu quan trọng lên Cloudflare R2 theo lịch, hỗ trợ mã hóa tùy chọn, xoay vòng, chạy tay và dọn dẹp lịch sử.</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Badge variant={backupState?.running ? "warning" : backupState?.last_status === "success" ? "success" : "secondary"} className="rounded-md">
                {backupState?.running ? "Đang backup" : backupState?.last_status === "success" ? "Gần nhất thành công" : backupState?.last_status === "error" ? "Gần nhất thất bại" : "Chưa chạy"}
              </Badge>
            </div>
          </div>

          <div className="rounded-xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-600">
            Tài khoản và khóa người dùng sẽ được xuất thành snapshot logic từ backend lưu trữ hiện tại, không phụ thuộc vào `json`, `sqlite`, `postgres` hay `git`. Thư mục hình ảnh mặc định không backup để tránh gói backup quá lớn.
          </div>
          ...
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Tiền tố backup</label>
            ...
            <p className="text-xs text-stone-500">Tiền tố object trong R2, ví dụ `backups/prod`.</p>
          </div>
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Chu kỳ backup</label>
            ...
            <p className="text-xs text-stone-500">Đơn vị phút; sau khi dịch vụ khởi động sẽ tự động polling và chạy theo chu kỳ này.</p>
          </div>

          <div className="space-y-2">
            <label className="text-sm text-stone-700">Số lượng backup giữ lại</label>
            ...
            <p className="text-xs text-stone-500">Sau khi upload thành công, hệ thống sẽ tự động xóa các backup cũ hơn. Nhập `0` để tắt xoay vòng tự động.</p>
          </div>
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Mật khẩu mã hóa</label>
            <Input type="password" value={String(backup.passphrase || "")} onChange={(event) => setBackupField("passphrase", event.target.value)} placeholder={backup.encrypt ? "Bật mã hóa thì bắt buộc nhập" : "Để trống"} className="h-10 rounded-xl border-stone-200 bg-white" />
            <p className="text-xs text-stone-500">Chỉ dùng khi bật mã hóa. Hãy lưu giữ cẩn thận, nếu không bạn sẽ không giải mã được nội dung backup.</p>
          </div>
          </div>

          <div className="space-y-3 rounded-xl border border-stone-200 bg-white px-4 py-4">
          <div>
            <div className="text-sm font-medium text-stone-800">Nội dung backup</div>
            <p className="mt-1 text-xs text-stone-500">Tích chọn dữ liệu cần đưa vào gói backup theo từng thành phần.</p>
          </div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 className="text-sm font-medium text-stone-800">Lịch sử backup</h3>
              <p className="text-xs text-stone-500">Hỗ trợ xem thông tin object và xóa backup trên remote trực tiếp.</p>
            </div>
          </div>
          ...
          {isLoadingBackups ? (
            ...
            <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
              Chưa có bản ghi backup từ xa. Sau khi lưu cấu hình và chạy backup thủ công một lần, mục này sẽ hiển thị ở đây.
            </div>
          ) : (
            <div className="space-y-3">
              {backups.map((item) => {
        ...
        <DialogContent className="flex max-h-[85vh] max-w-3xl flex-col overflow-hidden rounded-2xl border-white/80 bg-white">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
            {detailLoading ? (
              ...
              <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
                Tạm thời không đọc được chi tiết backup; nếu đây là backup đã mã hóa, vui lòng xác nhận bạn đã nhập đúng mật khẩu mã hóa và lưu cấu hình trước.
              </div>
            ) : (
              <>
                <div className="grid gap-3 rounded-xl border border-stone-200 bg-stone-50 px-4 py-4 text-sm text-stone-600 md:grid-cols-2">
                  <div>
                    <div className="text-xs text-stone-500">Ten object</div>
                  ...
                  <div>
                    <div className="text-xs text-stone-500">Cach kich hoat</div>
                    <div className="mt-1 font-medium text-stone-800">{detail.trigger || "—"}</div>
                  </div>
                  <div>
                    <div className="text-xs text-stone-500">Phien ban ung dung</div>
                ...
                <div className="space-y-3">
                  <h4 className="text-sm font-medium text-stone-800">Noi dung tep</h4>
                  <div className="space-y-2">
                    {detail.files.map((item) => (
                      <div key={item.name} className="rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm">
                        ...
                        <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-stone-500">
                          <span>{item.exists ? "Da bao gom" : "Thieu"}</span>
                          <span>Kich thuoc {formatBytes(item.size)}</span>
                ...
                <div className="space-y-3">
                  <h4 className="text-sm font-medium text-stone-800">Noi dung snapshot</h4>
