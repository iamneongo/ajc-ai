...
import { ChatWorkspace } from "@/app/image/components/chat-workspace";
...
function deriveTurnStatus(
  ...
  if (failedCount > 0) {
    return {
      status: "error",
      error: `Có ${failedCount} ảnh tạo không thành công`,
    };
  }
  if (successCount > 0) {
    ...
    const turns = conversation.turns.map((turn) => {
      ...
      const images = turn.images.map((image) => {
        ...
        return {
          ...
          error:
            "Trang đã được tải lại hoặc tác vụ bị gián đoạn, không tìm thấy task ID để khôi phục",
        };
      });
      const derived = deriveTurnStatus({ ...turn, images });
      if (
...
function ImagePageContent({ isAdmin }: { isAdmin: boolean }) {
  ...
  const deleteConfirmDescription =
    deleteConfirm?.type === "all"
      ? "Bạn có chắc muốn xóa toàn bộ lịch sử hình ảnh không? Hành động này không thể khôi phục."
      : deleteConfirm?.type === "prompt"
        ? "Bạn có chắc muốn xóa lịch sử prompt này không? Kết quả tạo ảnh tương ứng vẫn được giữ lại."
        : deleteConfirm?.type === "results"
          ? "Bạn có chắc muốn xóa kết quả tạo ảnh này không? Lịch sử prompt tương ứng vẫn được giữ lại."
          : deleteConfirm?.type === "one"
            ? "Bạn có chắc muốn xóa hội thoại hình ảnh này không? Hành động này không thể khôi phục."
            : "";
      ...
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Đọc lịch sử hội thoại thất bại";
        toast.error(message);
      } finally {
        if (!cancelled) {
          ...
          images:
            part === "results"
              ? turn.images.map((image) => ({
                  id: image.id,
                  status: "error" as const,
                  error: "Kết quả tạo ảnh đã bị xóa",
                }))
              : turn.images,
        };
  ...
  const appendReferenceImages = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Đọc ảnh tham chiếu thất bại";
      toast.error(message);
    }
  }, []);
  ...
  const handleContinueEdit = useCallback(
    async (
      ...
      try {
        ...
        textareaRef.current?.focus();
        toast.success(
          "Đã thêm vào ảnh tham chiếu hiện tại, tiếp tục nhập mô tả để chỉnh sửa",
        );
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "Đọc ảnh kết quả thất bại";
        toast.error(message);
      }
    },
    [],
  ...
  const handleReuseTurnConfig = useCallback(
    async (conversationId: string, turnId: string) => {
      ...
      textareaRef.current?.focus();
      toast.success("Đã áp dụng lại cấu hình prompt này");
    },
    [],
  );
    ...
    async (conversationId: string) => {
      ...
      try {
        ...
        if (activeTurn.mode === "edit" && referenceFiles.length === 0) {
          throw new Error(
            "Không tìm thấy ảnh tham chiếu để tiếp tục chỉnh sửa",
          );
        }
        ...
        const message =
          error instanceof Error ? error.message : "Tạo ảnh thất bại";
        await updateConversation(conversationId, (current) => {
          const conversation = current ?? snapshot;
          return {
            ...conversation,
  ...
  const handleSubmit = async () => {
    ...
    if (targetStats.running > 0 || targetStats.queued > 1) {
      toast.success("Đã thêm vào hàng đợi của hội thoại hiện tại");
...
export default function ImagePage() {
  ...
  return (
    <div className="mx-auto flex h-[calc(100dvh-6.5rem)] min-h-0 w-full max-w-[1380px] flex-col gap-3 px-0 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] sm:h-[calc(100dvh-5.25rem)] sm:px-3 sm:pb-6">
      <ChatWorkspace />
    </div>
  );
}
