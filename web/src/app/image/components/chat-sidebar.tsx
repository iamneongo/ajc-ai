...
function promptCount(conversation: ChatConversation) {
  return conversation.messages.filter(
    (message) => message.role === "user" && message.content.trim(),
  ).length;
}

function pendingCount(conversation: ChatConversation) {
  return conversation.messages.filter((message) => message.status === "pending")
    .length;
}
        ...
        >
          {isLoadingHistory ? (
            ...
            <div className="px-2 py-3 text-sm leading-6 text-stone-500">
              Chưa có lịch sử trò chuyện. Sau khi gửi tin nhắn, hội thoại sẽ
              hiển thị tại đây.
            </div>
          ) : (
            conversations.map((conversation) => {
              const active = conversation.id === selectedConversationId;
              const pending = pendingCount(conversation);
              return (
                <div
                  key={conversation.id}
                  className={cn(
